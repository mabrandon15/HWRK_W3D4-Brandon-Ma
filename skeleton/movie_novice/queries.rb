...
def below_average_years
  ...
  Movie.select('yr', 'COUNT(*) as bad_movies').where('movies.score < 5').group('yr').order('bad_movies')
end
...
def uma_movies
  ...
  Movie.select('id', 'title', 'yr').joins(:actors).where("actors.name = 'Uma Thurman'").order('actors.name')
end
